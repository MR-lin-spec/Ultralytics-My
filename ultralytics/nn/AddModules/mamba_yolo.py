...
def cross_selective_scan(
    ...
    # 确保所有参数权重都在同一设备上
    if x_proj_weight.device != device:
        x_proj_weight = x_proj_weight.to(device)
    if x_proj_bias is not None and x_proj_bias.device != device:
        x_proj_bias = x_proj_bias.to(device)
    if dt_projs_weight.device != device:
        dt_projs_weight = dt_projs_weight.to(device)
    if dt_projs_bias.device != device:
        dt_projs_bias = dt_projs_bias.to(device)
    if A_logs.device != device:
        A_logs = A_logs.to(device)
    if Ds.device != device:
        Ds = Ds.to(device)
    
    # 关键修复：确保out_norm层在正确的设备上
    ...
    def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
        # 确保所有输入都在CUDA上（selective_scan_cuda_core要求）
        if not u.is_cuda:
            u = u.cuda()
        ...
        if not C.is_cuda:
            C = C.cuda()
        if D is not None and not D.is_cuda:
            D = D.cuda()
        if delta_bias is not None and not delta_bias.is_cuda:
            delta_bias = delta_bias.cuda()
